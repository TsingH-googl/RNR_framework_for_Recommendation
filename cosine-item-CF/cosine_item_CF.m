...
train = (load('D:\链路预测相关课题\基于用户-物品的推荐\时间上下文信息数据集\MovieLens\ml-100k\u3.base'));
test = (load('D:\链路预测相关课题\基于用户-物品的推荐\时间上下文信息数据集\MovieLens\ml-100k\u3.test'));% 不知道为什么，这里的test在第460多位用户之后没有相关数据！
train = sortrows(train , 4);
test = sortrows(test , 4);
train(:,5) = formatdate(train(:,4)); % 时间数据按照天来算
test(:,5) = formatdate(test(:,4)); % 时间数据按照天来算

%% 根据训练集，基于余弦相似度生成item-similarity
userNum = max(data(:,1));
itemNum = max(data(:,2));
...
sim = cosine_item_similarity( ratingTrain_norm);

%% 根据item-similarity计算每个用户对所有item的probability，保存在score稀疏矩阵
%% 注意，这里既要计算买过的item也要算没有买过的item，因为后面要计算existing 与nonexisting的分数分布
K = 10; % 选择前K个与当前item物品最相似的前K个item
score = itemCF( ratingTrain, sim, userNum, itemNum, K);
...
score_nonexisting = sparse(userNum, itemNum,0);
...
%                     pearson_user_CF : % u1:41.49%, u2; 36.59% , u3: ,       u4:33.73% , u5:34.74% 。
                      % u1:39.69%, u2; 36.45% , u3: 34.36%, u4:33.76% , u5:  。
