...
X = load(' D:\链路预测相关课题\基于用户-物品的推荐\时间上下文信息数据集\lastfm-2k\user_taggedartists-timestamps.dat'); % esccort好像在CF没那么有效
...
pH = 0.1;       % 测试集占比
winSize = 7;    % PNR滤波窗口大小
bin_array = [5 10 20 40 60 100 150 200];        % 计算PNR的bin数目
...
%% CF参数
K = 40;          % 原文为K=50左右；计算user-item similarity的参数，对结果的影响也很大，K增大会使得PNR结果变优！！K太小则呈山峰爬升状而无效果
% wikibooks取35
%% FISM参数
FISM_n_lfactors = 128;% 原文2-32上升，96后稳定ml-100k为96（50效果好一点），FISM该这两个参数好像变化不大（ML1M时间较长）16+0.5则ML1M出错
FISM_alpha = 0.5; % 原文中ml-100k、netflix、yahoo为0.4-0.5（如果程序出错，肯定是跟这里有关）
%% IBPR参数
IBPR_maxIter = 50;% 迭代次数，一般是25的整数倍
IBPR_d = 30; % U、V特征向量的维度数
%% MatricCompletion
MC_mu = 0.03;% 越小则ML100K越不好，mu = [0.001,0.01]不一定哦,ML100K在0.0006时候score为NAN;Wikibooks670.001变小会NAN、0.006不好
MC_rho = 1.5;% rho = [1,5]，2对wikibooks0.9不好;变小对整体十分不好
MC_maxiter = 50;% 默认为1000次（小规模网络跟50次的速度差不多），50对wikibooks0.9buhao，ML100K0.9加大到100稍微改进.
%% PureSVD的参数
...
%% NMF参数
nmf_k = 20; % NMF原文ML100K为10-20，ML100K10较好;wikibooks20则总体好但是效果差
nmf_maxit = 50;% NMF原文ML100K为maxit为30-50，ML100K30较好
%% PMF参数
PMF_maxepoch = 50; % ml1m是50，原文中netflix max_epoch为50则比较好
PMF_num_feat = 30;% ml1m是10，原文netflix num_feat为10\20\30D
%% BPMF参数
BPMF_maxepoch = 50;% ml1m默认是50
BPMF_num_feat = 30;% ml1m默认是10，ml100k5较好（原文netflix默认为feat30/20，epoch20-50）
%% basicMF参数
basicMF_lambda = 0.01;% （lastFM改这里影响不大原文ML100K为[0.01,0.1, 1,2,4,8,10,20,30,40,60,80，为10-20时最好）,改小对我们better,但是总体变low，变大变小总体不大，但是一直好
basicMF_feat_num = 15;% (lastFM改这里对结果影响很大)默认为10(原文为8-20)，5或30、40都好加大这个laskfm会有效，不过耗时大
basicMF_maxiter  = 100; %不过好像50与100；原文为500次，默认为20或100，lastfm变小迭代次数，我们的方法没有优势

%% 在不同的bin下进行评估
bin_arhr = zeros(length(bin_array) + 1 ,length(topL_array));
bin_recall = zeros(length(bin_array) + 1,length(topL_array));
bin_hr = zeros(length(bin_array) + 1,length(topL_array));
bin_precision = zeros(length(bin_array)+ 1 ,length(topL_array));
bin_RS = zeros(length(bin_array) + 1 , 1);
...
method = 'FISM';
savedir = 'C:\Users\zmy201\Desktop\实验数据\Parameter Sensitivity\LastFM\';

%% 时序网络划分数据集,0-1图
[train, probe] = timeDivideSeg(data, train_start_ratio, train_end_ratio, probe_start_ratio, probe_end_ratio);

% ARHR没效果，改这里也没多大作用
deg_index = find(sum(train, 2) > 2); % 去掉user。总体感觉user去5个效果才好，而item最好是0或者5；100k去5/0;在cosine_user下去掉小度用户后，效果好很多!!MC去掉小度用户也好很多
...
for bin_index = 1:1:length(bin_array)
    bin = bin_array(bin_index);
    for iter = 1 : pnriter
...
%         score = itemCF( train, sim, K); % 4300*5900需要6分钟
    ...
    %% 计算ARHR\recall\precision\hr\rs的均值
    bin_arhr(1 ,:) = arhrResult(1,:);
    bin_recall(1 ,:) = recall(1,:);
    bin_hr(1 ,:) = hrResult(1,:);
    bin_precision(1 ,:) = precision(1,:);
    bin_RS(1) = RS;
    
    bin_arhr(bin_index + 1 ,:) = calAve(arhrResult);
    bin_recall(bin_index + 1,:) = calAve(recall);
    bin_hr(bin_index + 1,:) = calAve(hrResult);
    bin_precision(bin_index + 1 ,:) = calAve(precision);
    bin_RS(bin_index + 1) = sum(RS_all(:,2)) / iter;
...
plotBin( topL_array, bin_arhr, bin_array);
