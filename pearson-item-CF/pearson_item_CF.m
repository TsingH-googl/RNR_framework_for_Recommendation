...
test = (load('D:\链路预测相关课题\基于用户-物品的推荐\时间上下文信息数据集\MovieLens\ml-100k\u4.test'));% 不知道为什么，这里的u1 test在第460多位用户之后没有相关数据！
...
sim_flash = pearson_item_similarity( ratingTrain);
...
score = itemCF( ratingTrain, sim_flash, userNum, itemNum, K);
...
% 计算precision，根据测试集中"每个用户缺的数目"进行推荐，即TopN推荐
